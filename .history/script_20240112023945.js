...
const sCapitulo = document.querySelector('#m-funcao');
const sVersiculo = document.querySelector('#m-funcao');
...
function openModal(edit = false, index = 0) {
    ...
    if (edit) {
        ...
        sAuxiliar.value = itens[index].auxiliar;
        id = index;
    } else {
        sNome.value = '';
        sLivro.value = '';
...
function saveItem() {
    ...
    if (id !== undefined) {
        ...
        itens[id].auxiliar = sAuxiliar.value;
    } else {
        itens.push({
            'nome': sNome.value,
            'livro': sLivro.value,
